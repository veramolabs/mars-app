...
function ImportView(props: any) {
  ...
  const handleSaveMessage = () => {
    if (message) {
      setLoading(true)
      agent.dataStoreSaveMessage({message})
      ...
      <Grid container spacing={2} justify="center">
          <Grid item xs={12}>
            ...
            <ButtonGroup variant="text" color="primary" aria-label="contained primary button group">
              <Button
                onClick={() => { setData(undefined); setShowUploadDialog(true)}}
                >Upload</Button>

              <Button
                onClick={() => { setData(undefined); setShowScanner(true)}}
                >Scan</Button>
          ...
          {showScanner && !data && <Grid item xs={12} >
